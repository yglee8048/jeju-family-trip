...
function FlightCard({flight, lang}: { flight: Flight; lang: Lang }) {
    ...
    return (
        <div
            className={`rounded-2xl p-4 shadow-sm ${isReturn ? 'bg-orange-50 border border-orange-100' : 'bg-sky-50 border border-sky-100'}`}>
            <div className="flex items-center justify-between mb-2">
        <span
            className={`text-xs font-bold px-2 py-0.5 rounded-full ${isReturn ? 'bg-orange-200 text-orange-700' : 'bg-sky-200 text-sky-700'}`}>
...
export default function HomePage() {
    ...
    if (!persona) {
        return (
            <div className="min-h-screen bg-linear-to-b from-sky-50 to-white animate-fade-in">
    ...
    if (phase === 'pre') {
        ...
        return (
            <div className="px-4 pt-4 space-y-5">
                {/* D-Day Card */}
                <div
                    className="relative bg-linear-to-br from-[#9CD5FF] via-sky-400 to-sky-500 rounded-3xl p-6 text-white overflow-hidden animate-fade-in">
        ...
        <div className="px-4 pt-4 space-y-5">
            ...
            {todayTimeline.length > 0 && (
                <div className="animate-fade-in stagger-1">
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <Link href={`/schedule/${currentDay}`} className="text-xs text-sky-600 font-medium">전체 보기
                            →</Link>
                    </div>
            ...
            {filteredChecklist.length > 0 && (
                <div className="animate-fade-in stagger-2">
                    ...
                    <div
                        className="bg-white rounded-2xl divide-y divide-gray-100 shadow-sm border-l-4 border-amber-400">
